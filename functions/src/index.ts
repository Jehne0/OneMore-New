...
import { flexibleWeeklyWindowForServer } from "./flexibleWeeklyCore";
...
function flexibleWeeklyWindow(challenge: Record<string, any>, dateISO: string) {
  return flexibleWeeklyWindowForServer(challenge, dateISO);
}
...
function personalDateIsActive(challenge: Record<string, any>, dateISO: string): boolean {
  ...
  if (inactive.some((period: any) => String(period?.startDate ?? "") <= dateISO
      && (!period?.endDate || dateISO < String(period.endDate)))) return false;
  if (challenge.period === "flexibleWeekly") return flexibleWeeklyWindow(challenge, dateISO) !== null;
  return widgetDateIsActive({ ...challenge, status: "active" }, dateISO);
}

function serverEasyMode(state: Record<string, any>, challenge: Record<string, any>, challengeId: string): boolean {
...
function serverFlexiblePeriodFullyActive(challenge: Record<string, any>, start: string): boolean {
  for (let offset = 0; offset < 7; offset += 1) {
    if (!personalDateIsActive(challenge, addIsoDays(start, offset))) return false;
  }
  return true;
}

function reconcileServerFlexibleWeekly(
  state: Record<string, any>,
  challengeId: string,
  dateISO: string,
): { state: Record<string, any>; challenge: Record<string, any>; history: Record<string, any>[] } {
  let challenge = (Array.isArray(state.challenges) ? state.challenges : [])
    .find((item: any) => String(item?.id ?? "") === challengeId) as Record<string, any>;
  ...
  const challenges = (Array.isArray(state.challenges) ? state.challenges : [])
    .map((item: any) => String(item?.id ?? "") === challengeId ? challenge : item);
...
async function completePersonalChallengeForWidget(
  ...
  return db.runTransaction(async (tx) => {
    ...
    let state = document.state as Record<string, any> | undefined;
    ...
    let history = Array.isArray(state.history) ? state.history as Record<string, any>[] : [];
    ...
    if (count >= target || (flexibleWindow && completedToday > 0)) return { status: "already-completed", count };
