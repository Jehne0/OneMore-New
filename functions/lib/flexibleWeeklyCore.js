...
exports.flexibleWeeklyWindowForServer = flexibleWeeklyWindowForServer;
...
function flexibleWeeklyWindowForServer(challenge, dateISO) {
    ...
    if (pending && !usingPending) {
        ...
        if (!DATE_RE.test(previousPeriodEnd) && DATE_RE.test(oldFirst) && effective > oldFirst) {
            const elapsedBeforeEffective = Math.floor((Date.parse(`${effective}T00:00:00Z`) - Date.parse(`${oldFirst}T00:00:00Z`)) / 86400000) - 1;
        ...
        if (DATE_RE.test(previousPeriodEnd) && dateISO > previousPeriodEnd)
            return null;
    }
    const first = usingPending ? effective : String(challenge.flexibleWeeklyFirstPeriodStart ?? "");
    if (!DATE_RE.test(first) || dateISO < first)
        return null;
    ...
    const elapsed = Math.floor((Date.parse(`${dateISO}T00:00:00Z`) - Date.parse(`${first}T00:00:00Z`)) / 86400000);
